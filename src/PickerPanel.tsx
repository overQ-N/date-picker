...
import {
  MONTH_COL_COUNT,
  type MonthCellRender,
} from "./panels/MonthPanel/MonthBody";
...
function PickerPanel<DateType>(props: PickerPanelProps<DateType>) {
  ...
  const { inRange, panelPosition, rangedValue, hoverRangedValue } =
      ...
      switch (key) {
        ...
        case KeyCode.DOWN:
          date = operationFnc(
            ...
            Number(
              `${key === KeyCode.UP ? "-" : ""}${
                isDateMode ? WEEK_DAY_COUNT : MONTH_COL_COUNT
              }`
            )
          );
          break;
    ...
    >
      <div
        ...
        className={classNames(
          "tw-inline-flex tw-flex-col tw-text-center tw-bg-gray-0 tw-border tw-border-gray-1 tw-outline-none tw-rounded-sm",
          `${prefixCls}-panel`,
          className,
          {
